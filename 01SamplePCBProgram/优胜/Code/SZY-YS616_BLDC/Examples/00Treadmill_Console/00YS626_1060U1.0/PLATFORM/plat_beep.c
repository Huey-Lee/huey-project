...
void Beep_Init(void) {
    BSP_PWM_Init();
    g_beep_dev.mute_flag = 0; // 默认开启0, 关闭1
}
...
void Beep_Play(BeepMode_t mode) {
    switch (mode) {
        case BEEP_CLICK:    Beep_Play_Custom(2700, 50, 50, 0, 1);    break;
        case BEEP_LIMIT:    Beep_Play_Custom(4000, 50, 200, 0, 1);   break;
        case BEEP_ALARM:    Beep_Play_Custom(2700, 50, 100, 100, 10);break;
        case BEEP_POWER_ON: Beep_Play_Custom(2000, 50, 400, 0, 1);   break;
    }
}
...
void Beep_Handler_1ms(void) {
    ...
    if (g_beep_dev.timer > 0) {
        ...
        if (g_beep_dev.is_playing) { // 响完了一个周期
            BSP_PWM_Stop();
            g_beep_dev.is_playing = 0;
            g_beep_dev.timer = g_beep_dev.off_ms;
            if (g_beep_dev.off_ms == 0) g_beep_dev.count = 0; // 无间隔直接结束
        } else { // 停完了一个周期
