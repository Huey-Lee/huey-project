...
bit old_bit;                   //保存上一次 查询到的电平状态
bit syn_ok;                    //同步码标志位 
bit rf_ok;                     //接收完整命令标志位，通知解码程序可以解码了
bit rf_ok_first,rf_ok_second;  //接收成功临时标志
    ...
    if(!old_bit)   //检测到从低到高的跳变，已检测到一个完整的（高-低）电平周期
    {
      ...
      {        
        ...
        if(rec_bit_cnt >= RF_REC_BIT_LEN)  //接收完32位数据
        {
          if(!rf_ok_first)
          { 
            if((ee_param.key_add1 == 0)&&(ee_param.key_add2 == 0)) //遥控对码
            {
            ...
            {
              first_code1=ee_param.key_add1;
              first_code2=ee_param.key_add2;
              if(first_code1 != temp_code1 || first_code2 != temp_code2){//其他遥控器匹配的第一次接收
                  match_first_code1 = temp_code1; 
                  match_first_code2 = temp_code2;
              }
            }
...
//          case 32: { temp_code4=temp_code4 | 0x01; 
                    ...
                    if(!rf_ok_first)
                    {
                      ...
                      syn_ok=0;
                      rec_bit_cnt=0;
                      s_tim=900;       //1200      //2500; //晚点调整大小   2500*20=50ms                    
                      break;
                    }
                    else
                    {
  ...
  {       
    ...
    if(!s_tim) rf_ok_first=0;
		if(temp_code1==first_code1&&temp_code2==first_code2)
  ...
  {         
    ...
    if((first_code1==second_code1)&&(first_code2==second_code2)&&(first_code3==second_code3)){
      if((ee_param.key_add1 == 0)&&(ee_param.key_add2 == 0)) //遥控对码
        {
          ee_param.key_add1 = first_code1;
          ee_param.key_add2 = first_code2;  
        }
        
//        key_value = first_code3;
				
				if(short_long_keytime<3)
					short_long_keytime++;
    }
    else if((match_first_code1==second_code1)&&(match_first_code2==second_code2)&&(first_code3==second_code3)){//收到其他遥控器的对码操作
        match_key_value = first_code3;
    }
