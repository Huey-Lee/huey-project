...
void treadmills_proc_loop(void) //处理
{
  ...
  static u8 val=0;
      ...
      val=1;
        ...
        if(val<=2)  //启动倒计时次数
        {
          ...
          val++;
				...
				aerolink_send_start(treadmills.param.speed); /* AeroLink启动+速度，retry由aerolink_ctrl_loop负责 */
				/* (重发由 aerolink_ctrl_loop 每500ms保底) */
	...
	case STATUS_STOP: 		
		...
		aerolink_send_stop(); /* AeroLink停止命令，retry由aerolink_ctrl_loop负责 */
		/* (重发由 aerolink_ctrl_loop 每500ms保底) */
    set_seg_mode(SEG_MODE_NORMAL);
...
//      set_seg_mode(SEG_MODE_FLASH_SELF);
  ...
  if(error_times==0)
	{
		aerolink_send_stop(); /* 故障时发送AeroLink停止命令 */
...
//    set_bit_seg(0,SEG_ALL_OFF);//强制关闭显示
//    set_bit_seg(1,SEG_ALL_OFF);//强制关闭显示
		if(treadmills.param.speed<1000)
    {
			set_bit_seg(0,SEG_ALL_OFF);//强制关闭显示
      set_bit_seg(1,SEG_ALL_OFF);//强制关闭显示
		}
...
void communication_checkout(void)
{
    ...
    case 0:
      treadmills.ack=0;
      /* heartbeat handled by aerolink_ctrl_loop */
      timeout=0;
      sm=1;
      break;
    case 1:
