...
void send_default_param(void)
{
  ...
  uart1_frame_tx(CMD_OVER_CURRENT_MAX,treadmills.param.over_current_max);
	...
	uart1_frame_tx(CMD_STA_LEVEL,5);					//value chose betwen 1~5 , the higher the level , the faster the starting speed
	uart1_frame_tx(CMD_END_LEVEL,25);					//VALUE CHOSE BETWEN 2~25 , the higher the level , the faster the ending   speed
	uart1_frame_tx(CMD_Accelerated_LEVEL,5);
  uart1_frame_tx(CMD_deceleration_LEVEL,5);
        ...
        ms500_flag=0;
				...
				if(poweron_flag == 1)
          {
            ...
            if(poweron_cnt == 1)   											//全亮2秒后显示版本号
            {
              ...
              set_bit_seg(1,NUM_2);			//版本号高位
              set_bit_seg(2,NUM_1);			//版本号低位
