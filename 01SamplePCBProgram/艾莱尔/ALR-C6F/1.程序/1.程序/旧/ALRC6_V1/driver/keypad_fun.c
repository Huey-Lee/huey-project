...
static void btn_sub_proc(int scale)
{
  if(scale==1)
  {
    if(treadmills.status==STATUS_RUNNING)
    {
     if(treadmills.param.speed>TREADMILLS_SPEED_MIN)//100
      {
        beep_set(1,DEFAULT_ON_TICKS,DEFAULT_OFF_TICKS);
        treadmills.param.speed -= treadmills.param.speed_step;  //2.2
        if( treadmills.param.speed < TREADMILLS_SPEED_MIN)
        {
          treadmills.param.speed = TREADMILLS_SPEED_MIN;           //避免减了步距之后小于最小值2023.2.2
        }
      }
      else{treadmills.param.speed=TREADMILLS_SPEED_MIN;}
...
//      current_mode_index = (current_mode_index + 1) % 3; // 循环切换速度模式  
