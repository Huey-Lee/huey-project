...
void send_default_param(void)
{
  ...
  uart1_frame_tx(CMD_OVER_CURRENT_MAX,treadmills.param.over_current_max);
	...
	uart1_frame_tx(CMD_STA_LEVEL,3);					//value chose betwen 1~5 , the higher the level , the faster the starting speed
	uart1_frame_tx(CMD_END_LEVEL,20);					//VALUE CHOSE BETWEN 2~25 , the higher the level , the faster the ending   speed
    ...
    {				
     ...
     //100ms 周期处理程序
      if(ms100_flag)
      {
        ...
        bt_power_send();
        aerolink_ctrl_loop();  /* AeroLink心跳+启停命令可靠重发 */
      }
