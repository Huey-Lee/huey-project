...
/* Open-loop start ramp: voltage target ceiling in ADC units (~51 V sense).
 * Prevents adjust_sta_voltage from climbing indefinitely if the motor
 * fails to start (belt jammed / open circuit). E03 is then triggered by
 * checkout_start_stall() timeout before or at this cap. */
...
/* E01 link-loss, E02 bus-unbalance, E03 over-current/stall, E04 relay,
 * E05 over-voltage, E06 over-speed/overshoot/e-stop, E08 MOS-short */
typedef enum _error_code_enum
{
	ERROR_01=1,   /* communication loss                    → soft ramp stop   */
	ERROR_02,     /* bus voltage unbalanced                → immediate cut    */
	ERROR_03,     /* over-current / stall                  → immediate cut    */
	ERROR_04,     /* relay fault (reserved)                                   */
	ERROR_05,     /* bus over-voltage                      → immediate cut    */
	ERROR_06,     /* over-speed / volt-overshoot / e-stop  → immediate cut    */
	ERROR_07,     /* (unused)                                                 */
	ERROR_08,     /* MOS short-circuit                     → immediate cut    */
	ERROR_09,
	ERROR_0A,
	ERROR_0B,
	ERROR_0C,
