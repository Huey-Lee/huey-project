...
/* KIV 硬限幅：附加量 (ΔI*kiv/10) 不超过 adjust_now_voltage 的 NUM/DEN；NUM=0 表示不做比例限幅 */
#define KIV_VOLT_EXTRA_MAX_NUM   (12)
#define KIV_VOLT_EXTRA_MAX_DEN   (100)
/* 可选绝对上限（与 adjust_* 同一 u16 刻度，约 val*8.13）；0 表示关闭 */
#define KIV_VOLT_ADD_ABS_MAX     (0)

/* 送入 pid_calc 的电压目标每 ISR 最大变化量（~1.6 kHz）；0 表示关闭斜率限制 */
#define PID_TARGET_VOLT_SLEW_MAX (10)
...
/* E08：MOS 短路/击穿等导致母线相对 PID 给定异常抬高（与 pid_calc 同 u16 刻度） */
#define E08_MOS_FAULT_DETECT_EN       (1)
/* 实测 cut_adc 比 pid_target 高出此 ADC 差值以上才开始累计（约 25~35V 需按硬件标定 220~280） */
#define E08_ACT_ABOVE_TARGET_ADC      (240u)
/* 减速(set<cur)时母线可能高于给定，附加裕量；设 0 则不减敏 */
#define E08_REGEN_EXTRA_ADC           (100u)
#define E08_MOS_FAULT_DEBOUNCE        (45u)
/* 1=仅当 ZHANKONBI<=E08_PWM_ASSIST_MAX 才判（更保守）；0=不强制低占空比 */
#define E08_ASSIST_LOW_PWM_ONLY       (0)
#define E08_PWM_ASSIST_MAX            (130u)
/* 占空比单 ISR 异常上冲：超过 pid 允许步进(UK_STEP)+裕量则累计 E08（积分暴走/异常写占空等） */
#define E08_PWM_SPIKE_DETECT_EN       (1)
#define E08_PWM_SPIKE_STEP_MARGIN     (4u)
#define E08_PWM_SPIKE_DEBOUNCE        (2u)

/* E01 link, E02 open, E03 OC/上电母线过低, E04 relay, E05 OV, E06, E07, E08 MOS/母线失控抬高 */
typedef enum _error_code_enum
{
	ERROR_01=1,
	ERROR_02,
	ERROR_03,    /* E03: 过流、上电时母线过低等（原 E08 类） */
	ERROR_04,
	ERROR_05,   	/* E05: over-voltage */
	ERROR_06,     /* E06 */
	ERROR_07,    	/* E07 */
	ERROR_08,     /* E08: 母线异常高于给定 / 占空比单步异常上冲 */
	ERROR_09,
	ERROR_0A,
	ERROR_0B,
	ERROR_0C,
