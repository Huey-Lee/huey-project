...
static _Bool    s_boot_auto_accel;   /* auto-accelerate after boot start    */
static uint8_t  s_boot_auto_tick;    /* 100 ms tick divider                 */
...
static void Run_Tick(void)
{
    ...
    if (s_boot_auto_accel) {
        if (++s_boot_auto_tick >= TM_BOOT_AUTO_ACCEL_100MS) {
            s_boot_auto_tick = 0u;
            if (g_TM.speed < TM_SPEED_MAX) {
                uint32_t next = (uint32_t)g_TM.speed + TM_BOOT_AUTO_ACCEL_STEP;
                g_TM.speed = (next > TM_SPEED_MAX) ? TM_SPEED_MAX : (uint16_t)next;
                Send_Speed(g_TM.speed);
                UI_MarkDirty();
            }
            if (g_TM.speed >= TM_SPEED_MAX) {
