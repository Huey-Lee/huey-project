...
static const CfgRow_t s_cfg_table[] = {
    ...
    { CMD_OVER_CURRENT_LOW_SPEED_MAX, 1u, TM_LOWER_OVER_CURRENT_MAX_LOW_SPEED, 0u },
    ...
    { CMD_KIV_13KM,             1u, TM_LOWER_KIV_13KM,          0u },
    { CMD_KIV_14KM,             1u, TM_LOWER_KIV_14KM,          0u },
    { CMD_SOFT_START_VOLT,      1u, TM_LOWER_SOFT_START_VOLT,   0u },
    { CMD_SOFT_START_MS,        2u,
      (uint8_t)(((uint32_t)TM_LOWER_SOFT_START_S * 1000u) & 0xFFu),
      (uint8_t)((((uint32_t)TM_LOWER_SOFT_START_S * 1000u) >> 8u) & 0xFFu) },
    { CMD_SOFT_STOP_MS,         2u,
      (uint8_t)(((uint32_t)TM_LOWER_SOFT_STOP_S * 1000u) & 0xFFu),
      (uint8_t)((((uint32_t)TM_LOWER_SOFT_STOP_S * 1000u) >> 8u) & 0xFFu) },
    { CMD_SOFT_STOP_FLOOR_V,    1u, TM_LOWER_SOFT_STOP_FLOOR_V, 0u },
};
/* Explicit count avoids Clang "incomplete type" if any row fails to parse. */
enum { CFG_TABLE_ROWS = 30 };
...
static uint8_t  s_cfg_wait_ack;      /* waiting ack echo for row */
static uint8_t  s_cfg_ack_ok;        /* ack matched flag         */
static uint8_t  s_cfg_retry;         /* resend count for row     */
static uint8_t  s_cfg_ack_timeout;   /* 100 ms ticks while wait  */
/* 0=unknown(probe), 1=ack-capable, 2=legacy(no param echo ack). */
static uint8_t  s_cfg_ack_mode;
...
static void BootCfg_Poll(void)
{
    ...
    if (s_cfg_ack_mode == 2u) {
        /* Legacy lower board: no param echo ACK, keep old burst behavior. */
    ...
    if (++s_cfg_ack_timeout >= TM_LOWER_CFG_ACK_TIMEOUT_100MS) {
        s_cfg_ack_timeout = 0u;
        if (s_cfg_retry < TM_LOWER_CFG_ACK_RETRY_MAX) {
            s_cfg_retry++;
            CfgRow_Send(&s_cfg_table[s_cfg_step]);
        } else {
            if (s_cfg_ack_mode == 0u) {
                /* Probe failed: auto downgrade to legacy mode, no E01 here. */
                s_cfg_ack_mode = 2u;
                s_cfg_wait_ack = 0u;
                s_cfg_ack_ok = 0u;
                s_cfg_retry = 0u;
...
void AeroLink_OnFrameReceived(AeroRecvFrame_t *f)
{
    ...
    if (s_cfg_wait_ack && s_cfg_step < CFG_TABLE_LEN) {
        const CfgRow_t *row = &s_cfg_table[s_cfg_step];
        if (f->cmd == row->cmd && f->len == row->len) {
            if (row->len == 1u) {
                if (f->data[0] == row->d0) {
                    if (s_cfg_ack_mode == 0u) s_cfg_ack_mode = 1u;
                    s_cfg_ack_ok = 1u;
                }
            } else if (f->len >= 2u) {
                if (f->data[0] == row->d0 && f->data[1] == row->d1) {
                    if (s_cfg_ack_mode == 0u) s_cfg_ack_mode = 1u;
                    s_cfg_ack_ok = 1u;
                }
            }
