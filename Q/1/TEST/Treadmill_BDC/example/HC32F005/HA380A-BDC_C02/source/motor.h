...
/* E08: |指令 − cut_adc| raw (= V×8.13)。默认用 adjust_now_voltage；勿用 adjust_kiv_voltage（含 KIV）否则斜率限幅时误差会一直大 → 误报 */
#ifndef VOLT_MISMATCH_RAW
#define VOLT_MISMATCH_RAW           (163u) /* ~20 V 当量，可按台架放宽 */
#endif
#ifndef E08_COMPARE_USE_KIV
#define E08_COMPARE_USE_KIV         (0u) /* 1=与 PI 完全一致用 adjust_kiv_voltage（易与 pwm 斜率冲突误报） */
#endif
#ifndef E08_MISMATCH_DEBOUNCE_CNT
#define E08_MISMATCH_DEBOUNCE_CNT   (48u) /* 连续超差次数约数十毫秒级再跳闸，抑制抖动 */
#endif

#define MAX_MT_SPEED_SCALE          (120)        /* 12.0 in 0.1 units */
/* E01 link, E02 open, E03 OC (was E08), E04 relay, E05 OV, E06 over-spd, E07, E08 cmd vs motor V mismatch */
typedef enum _error_code_enum
{
	...
	ERROR_06,     /* E06 */
	ERROR_07,    	/* E07 */
	ERROR_08,     /* E08: adjust_now_voltage vs cut_adc sustained mismatch（可调 KIV 比较） */
	ERROR_09,
	ERROR_0A,
	ERROR_0B,
	ERROR_0C,
...
typedef struct _t_motor
{
  ...
  u8 adjust_cou_voltage;
	u16 adjust_kiv_voltage; /* PI 电压给定 = adjust_now_voltage + KIV 电流补偿（与 pid_calc 第一个参数一致） */
