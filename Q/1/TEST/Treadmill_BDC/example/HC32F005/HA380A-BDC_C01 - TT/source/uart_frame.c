...
//		 motor.sta_speed_scale=uart_frame.buf[0];
	...
	{ // start_valtage，U= start÷8.13
		...
		case(3): motor.start_valtage = 98;	motor.start_add_val = 5; break; /* ~12.0V */
  ...
  case CMD_SOFT_START_VOLT:
    {
      ...
      motor.cfg_ramp_start_volt = v;
      motor_recalc_start_ramp_interval();
    }
    break;
  case CMD_SOFT_START_MS:
    if (uart_frame.len >= 2u) {
      u16 ms = (u16)uart_frame.buf[0] | ((u16)uart_frame.buf[1] << 8);
      if (ms < 500u) {
        ms = 500u;
      }
      if (ms > 60000u) {
      ...
      motor.cfg_soft_start_ms = ms;
      motor_recalc_start_ramp_interval();
    }
    break;
  case CMD_SOFT_STOP_MS:
    if (uart_frame.len >= 2u) {
      u16 ms = (u16)uart_frame.buf[0] | ((u16)uart_frame.buf[1] << 8);
      if (ms > 60000u) {
        ms = 60000u;
      }
      motor.cfg_soft_stop_ms = ms;
      motor_recalc_stop_ramp_div();
    }
    break;
  case CMD_SOFT_STOP_FLOOR_V:
    {
      u8 v = uart_frame.buf[0];
      if (v == 0u) {
        ...
        if (v > motor.adjust_max_voltage) {
          v = motor.adjust_max_voltage;
        }
        motor.cfg_stop_floor_pwm = motor_drive_pwm_from_volt_u8(v);
      }
      motor_recalc_stop_ramp_div();
    }
    break;
