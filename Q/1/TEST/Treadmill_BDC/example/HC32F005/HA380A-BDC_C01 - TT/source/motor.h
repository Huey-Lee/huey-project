...
/* 缓停 ISR 递减到的 PWM 地板（与 motor_speed_pid 一致） */
#define MOTOR_STOP_PWM_FLOOR_DEFAULT (8u)
...
#define OVER_CURRENT_MAX_LOW_SPEED  			(CURRENT_7A0)   /* stricter at low speed */
...
/* E01 link, E02 open, E03 stall, E04 relay, E05 OV, E06 over-spd, E07, E08 OC/switch */
...
typedef struct _t_motor
{
  ...
  u8 adjust_cou_voltage;
	...
	/* 上控统一下发：缓起/缓停（与 CMD_SOFT_* 对应） */
	u8  cfg_ramp_start_volt;          /* 缓起起始电压 V，默认 7 */
	u16 cfg_soft_start_ms;            /* 缓起总时长 ms，0 表示 5000 */
	u16 cfg_start_ramp_interval_ticks;/* ISR 间隔，由 motor_recalc_start_ramp_interval 计算 */
	u16 cfg_soft_stop_ms;             /* 缓停总时长 ms，0 表示用 cfg_stop_ramp_div 默认 24 */
	u8  cfg_stop_ramp_div;            /* 停机 PWM 每减 1 的 ISR 分频，越大越慢 */
	u16 cfg_stop_floor_pwm;           /* 缓停占空地板，0 表示 MOTOR_STOP_PWM_FLOOR_DEFAULT */
}motor_t;
...
extern u16 motor_drive_pwm_from_volt_u8(u8 volts_v);
extern void motor_recalc_start_ramp_interval(void);
extern void motor_recalc_stop_ramp_div(void);
