...
static void  checkout_speed_low(void)
{
	...
	if(current_max_over>10){	//three hundred millisesecond
		ctr.status = NULL;
		motor.status = NULL;
		ctr.error_code=ERROR_08;
		ctr.status=STATUS_ERROR;
...
//	timestest++;
	switch(motor.status)
	{
		...
		start_val = (0.126*cut_adc);
		
		if(motor.start_tim%24==0)
		{
			motor.adjust_sta_voltage+=1;
			if(motor.adjust_sta_voltage>(u16)((motor.adjust_min_voltage+60)*8.13))
				motor.adjust_sta_voltage = (u16)((motor.adjust_min_voltage+60)*8.13);
		}
		...
		if(motor.start_tim<60000)
		motor.start_tim++;
		if(motor.start_tim >= 400 && (((float)motor.start_set_val-0.5)  < start_val))
		{
			motor.start_tim=0;
...
//				timestest = motor.adjust_kiv_voltage;
			motor.adjust_kiv_voltage = motor.adjust_now_voltage + motor.adjust_kiv_voltage;
		}
		else
			motor.adjust_kiv_voltage = motor.adjust_now_voltage;
		
		valtage = (u8)(0.126*cut_adc);
		ZHANKONBI = pid_calc(&mt_pid, motor.adjust_kiv_voltage,(u16)cut_adc,mot.index);
		
		if(motor.END_speed_scale > valtage)	//when time to end  ZHANKONBI almost 25
//		if(motor.END_speed_scale > ZHANKONBI)	//
		{
			motor.status = NULL;
