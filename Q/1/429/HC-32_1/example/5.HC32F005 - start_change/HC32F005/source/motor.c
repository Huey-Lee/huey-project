...
//	uint16_t cut_adc;			//ad差值
		...
		for(u8 i=0;i<100;i++)
		{
			...
			if(interrupt_flag!=adc_handle.irq_flag)//采集到数据
			{
				...
				if(adc_handle.irq_flag == 0 && interrupt_flag > 0)//采集完成
				{
					...
					if((val_up>(val_down + 30)) || (val_down > (val_up + 30)))
					{
						er02_code++;
					}
	...
	case STATUS_RUN:
		...
		{
			er02_code = 0;
      ctr.error_code=ERROR_06;								//电机断线
      ctr.status=STATUS_ERROR;
		}
		else if(er08_code > 5)
		{
			er08_code = 0;
      ctr.error_code=ERROR_02;							//立诚指定
      ctr.status=STATUS_ERROR;
		}
		else
		{
	...
	case STATUS_ERROR:
		...
		{
//    case ERROR_COMM://er01 没有通讯，上下板之间没有建立通讯
...
//			break;
		case ERROR_05://er05 电流超过保护值（电机负载过大）
			tim0stop();
			tim6stop();
...
void motor_speed(void)
{
	...
	switch(motor.status)
	{
		...
		if(motor.stop_valtage>10)
			motor.stop_valtage = motor.stop_valtage - 0.5;
		else
			motor.stop_valtage=10;
		val = (u16)GET_SPEED_VOLTAGE(motor.cur_speed_scale,motor.voltage_param,motor.stop_valtage);
		motor.adjust_now_voltage = (u16)(val*8.13);
