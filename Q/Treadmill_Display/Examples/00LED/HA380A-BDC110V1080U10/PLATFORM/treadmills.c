...
/* Adjusts the currently displayed target by dir (+1 / -1).
 * Clamps to min/max and beeps when a limit is reached.
 * Also sends the new speed live when RUNNING. */
static void Adjust_Target(int8_t dir, bool is_hold)
{
    ...
    switch (g_TM.disp_index) {
        ...
        case DISP_SPEED: {
            ...
            if (dir > 0) {
                if (g_TM.speed >= TM_SPEED_MAX) { at_limit = true; break; }
                uint32_t ns = (uint32_t)g_TM.speed + delta;
                g_TM.speed  = (ns > TM_SPEED_MAX) ? (at_limit = true, TM_SPEED_MAX)
                                                   : (uint16_t)ns;
            } else {
                if (g_TM.speed <= TM_SPEED_MIN) { at_limit = true; break; }
                int32_t ns = (int32_t)g_TM.speed - (int32_t)delta;
                g_TM.speed = (ns < TM_SPEED_MIN) ? (at_limit = true, TM_SPEED_MIN)
                                                  : (uint16_t)ns;
            }
        ...
        case DISP_TIME: {
            ...
            if (dir > 0) {
                ...
                g_TM.time   = (ns > TM_TIME_MAX) ? (at_limit = true, TM_TIME_MAX)
                                                  : ns;
            } else {
                if (g_TM.time <= TM_TIME_MIN) { g_TM.time = TM_TIME_MAX; break; }
                g_TM.time = (g_TM.time - TM_TIME_MIN < delta)
                            ? (at_limit = true, TM_TIME_MIN)
                            : g_TM.time - delta;
            }
            break;
        }
        ...
        case DISP_DIST: {
            ...
            if (dir > 0) {
                ...
                g_TM.dist = (nd > TM_DIST_MAX) ? (at_limit = true, TM_DIST_MAX) : nd;
            } else {
                if (g_TM.dist <= TM_DIST_MIN) { g_TM.dist = TM_DIST_MAX; break; }
                float nd = g_TM.dist - delta;
                g_TM.dist = (nd < TM_DIST_MIN) ? (at_limit = true, TM_DIST_MIN) : nd;
            }
            break;
        }
        ...
        case DISP_CAL: {
            ...
            if (dir > 0) {
                ...
                g_TM.cal = (nc > TM_CAL_MAX) ? (at_limit = true, TM_CAL_MAX) : nc;
            } else {
                if (g_TM.cal <= TM_CAL_MIN) { g_TM.cal = TM_CAL_MAX; break; }
                float nc = g_TM.cal - delta;
                g_TM.cal = (nc < TM_CAL_MIN) ? (at_limit = true, TM_CAL_MIN) : nc;
            }
            break;
        }
    ...
    if (at_limit) Beep_Play(BEEP_LIMIT);
}
    ...
    switch (g_TM.state) {
        ...
        case TM_STATE_IDLE:
            ...
            if (key_id == K_ID_ONOFF) {
                if (!Treadmill_LowerBoardReady()) { Beep_Play(BEEP_LIMIT); return true; }
