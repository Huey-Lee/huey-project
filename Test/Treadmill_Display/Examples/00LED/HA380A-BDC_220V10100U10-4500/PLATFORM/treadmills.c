...
static uint16_t s_stop_wait_cnt;     /* phase-2 only: 最低速到停机确认等待（tick） */
    ...
    switch (f->cmd) {
        ...
        case CMD_STOP_OVER:
            if (g_TM.state == TM_STATE_RUNNING || g_TM.state == TM_STATE_STOPPING) {
                /* 下位「彻底停止」：清减速/倒计时、速度清零、回待机（必须经过此帧）。 */
        ...
        case CMD_STATUS: {
            ...
            switch (status) {
                case 9u:   /* motor running */
                    ...
                    break;
                case 10u:  /* 兼容旧帧：非下位标准「彻底停止」路径，不替代 CMD_STOP_OVER */
                    break;
                case 3u:   /* motor fault */
                    g_TM.state = TM_STATE_ERROR;
                    break;
...
/* Phase 1：停止中 — 显示减速与 g_TM.timer_100ms 倒计时，不计入「最低速→停机」超时。
 * Phase 2：显示已为 0 — 仅在此阶段等待 CMD_STOP_OVER；超时未收到则 E01，不得回待机。 */
static void Stop_Tick(void)
{
    if (g_TM.timer_100ms > 0u) {
        ...
        UI_MarkDirty();
        s_stop_wait_cnt++;
        if (s_stop_wait_cnt >= TM_STOP_FULL_STOP_WAIT_MAX) {
            g_TM.state      = TM_STATE_ERROR;
            g_TM.error_code = 1u;
            Beep_Play(BEEP_ALARM);
        }
