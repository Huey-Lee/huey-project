/* user_usart.c - UART1 ring buffer, TX/RX, init, ISRs. */
...
uint8_t uart_send(uint8_t *data, uint8_t len)// uart ?????????????? printMsg ????????
{
...
{
	memset(uart1_rx_ringfifo_buffer,0,UART1_RX_RINGBUF_SIZE);																//????1????????????
...
//void uart_frame_loop(void)  //????????????? UART ????
//{
...
{
	unsigned int ret = 0;
	ret = ringfifo_get(&uart1_rx_ringfifo,buffer,len);//????????????????len????????buffer????
	return ret;																				//???????????????
}
