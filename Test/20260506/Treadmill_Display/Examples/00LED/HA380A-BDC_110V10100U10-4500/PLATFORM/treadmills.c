...
enum { CFG_TABLE_ROWS = 24 };
#define CFG_TABLE_LEN  ((uint8_t)CFG_TABLE_ROWS)

static uint8_t  s_cfg_step;          /* 当前发送行索引 */
static uint16_t s_cfg_delay_ticks;   /* 首次发送前延迟（100ms节拍） */
static uint8_t  s_cfg_gate_250ms;    /* 250ms门限计数 */
static uint8_t  s_cfg_done;          /* 1=全部行发完，允许启动 */
static uint8_t  s_cfg_new_fw;        /* 1=收到echo确认为新固件 */
...
/* Returns true once all 3 passes + verification are complete. */
_Bool Treadmill_LowerBoardReady(void)
{
    return s_cfg_done ? 1 : 0;
}
...
/* 100ms节拍：每次发 TM_LOWER_CFG_ROWS_PER_TICK 行，30行共约1.5s完成
 * 下控"发什么回什么"（echo）→ s_cfg_new_fw 标记新固件，不阻塞发送 */
static void BootCfg_Poll(void)
{
    if (s_cfg_done) return;
    if (s_cfg_gate_250ms < TM_LOWER_CFG_250MS_GATE) return;
    if (s_cfg_delay_ticks > 0u) { s_cfg_delay_ticks--; return; }

    for (uint8_t n = 0u; n < TM_LOWER_CFG_ROWS_PER_TICK; n++) {
        if (s_cfg_step >= CFG_TABLE_LEN) { s_cfg_done = 1u; break; }
        CfgRow_Send(&s_cfg_table[s_cfg_step++]);
    }
    if (s_cfg_step >= CFG_TABLE_LEN) s_cfg_done = 1u;
}
...
void AeroLink_OnFrameReceived(AeroRecvFrame_t *f)
{
    ...
    switch (f->cmd) {
        ...
        default:
            /* 配置命令echo（新固件回显cmd+data[0]）→ 标记为新固件 */
            if (f->cmd >= CMD_TREADMILLS_SPEED_MAX && f->len >= 1u) {
                s_cfg_new_fw = 1u;
            }
            break;
    }
...
static void Adjust_Target(int8_t dir, bool is_hold)
{
    ...
    switch (g_TM.disp_index) {
        ...
        case DISP_SPEED: {
            ...
            if (dir > 0) {
                if (g_TM.speed >= TM_SPEED_MAX) break; /* 已在上限，Keypad侧无UART故正常响 */
                uint32_t ns = (uint32_t)g_TM.speed + delta;
                g_TM.speed  = (ns >= TM_SPEED_MAX) ? TM_SPEED_MAX : (uint16_t)ns;
                /* 蜂鸣必须在 Send_Speed(UART) 之前：
                 *   UART 发送期间阻塞主循环 > 1ms，导致蜂鸣节拍打不满 → 断声/没响。
                 *   短按(PRESS)每次响；长按(HOLD)仅到达上限时响一声提示。 */
                if (!is_hold || g_TM.speed == TM_SPEED_MAX) {
                    Beep_Play(BEEP_CLICK);
                    s_suppress_keypad_beep = 1;
                }
            } else {
                if (g_TM.speed <= TM_SPEED_MIN) break; /* 已在下限，Keypad侧正常响 */
...
bool Treadmill_On_Event(uint8_t key_id, uint8_t evt)
{
    ...
    /* 全显期间及参数下发完成前，屏蔽所有按键（蜂鸣在 Keypad_On_Event 侧同步屏蔽） */
    if (!s_cfg_done) return true;
...
static void Data_Reset(void)
{
    ...
    g_TM.cycle_timer  = 0u;
    /* 不调 UI_ClearAll：IDLE render 会完整覆写所有 grid 和 indicator，
     * 避免 ClearAll→Display_Update 之间 LED 扫描采到空帧造成闪烁 */
}
...
static void Render_SingleZone(void)
{
    ...
    if (g_TM.state != s_prev_st || g_TM.disp_index != s_prev_disp) {
        /* IDLE render 会完整覆写所有 grid（SetNumber 4位）和 indicator（for循环全清）
         * 跳过 ClearAll 避免 LED 扫描中断在"清空→重绘"间隙采到空帧造成闪烁 */
