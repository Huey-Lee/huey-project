...
/* === 本工程出厂参数块（是否与电机/负载匹配请以台架体感与电流为准微调）===============
 * 「110V→STA 档 1 / 220V→档 2」见本节上方 ASCII 表格，仅为文档说明；
 *   本条仍用 STA_LEVEL 2 ——重载或大静摩擦皮带时较高起步档更易脱出粘滞，
 *   若低速明显「咯噔/抽动」可适当试降到 1，若起步无力或易抖在低速再试调高；无唯一正确答案。*/
#define TM_LOWER_VOLTAGE_MAX        90u    /* max drive voltage (V)        */
#define TM_LOWER_VOLTAGE_MIN        22u    	/* min drive voltage (V)，1km/h基础电压 */
...
/* CMD_STA_LEVEL(28)：见下控 uart_frame.c——1→(40,3)，2→(80,5)…档位越高起步目标电压越高、斜坡越快 */
...
/* --- KIV（CMD_KIV_1KM … 12KM）：整数 0~10
 *   增量 ≈ max(0, I滤波 − I空载基线) × KIV / 10（电压 ADC），另有约 +300 / +80（RUN）上限。
 *
 * 【推荐】低档↑K 上人更硬；4~6 km ↑K 中区更稳；8 km↑ ↓K 减电流噪声更安静。（与 H1 motor.c speed_param[].kiv 同步） */
#define TM_LOWER_KIV_1KM             8u
#define TM_LOWER_KIV_2KM             8u
#define TM_LOWER_KIV_3KM             7u
#define TM_LOWER_KIV_4KM             7u
#define TM_LOWER_KIV_5KM             7u
#define TM_LOWER_KIV_6KM             7u
#define TM_LOWER_KIV_7KM             7u
#define TM_LOWER_KIV_8KM             7u
#define TM_LOWER_KIV_9KM             6u
#define TM_LOWER_KIV_10KM            6u
 ...
 * 开启时：上电后 TM_RF_PAIR_WINDOW_10MS 内长按「速度加」累计 TM_RF_PAIR_HOLD_10MS，
 * 双短鸣后写入 Flash；已绑定后仅接受该地址。未绑定前任意合法遥控器可用。
 * 存储占 Flash 末页（plat_rf_store.c）；改为 1 前请确认固件不覆盖该页。
 *===========================================================================*/
#define TM_RF_PAIRING_ENABLE            0u
#define TM_RF_PAIR_WINDOW_10MS          600u   /* 6 s */
#define TM_RF_PAIR_HOLD_10MS            300u   /* 3 s 连续按住加键（10ms 节拍） */
...
#define TM_STOP_TIME_COUNT              350u
