 ...
 * Tim6：`Adt_SetCompareValue(..., OUT_PUT)`，且 **OUT_PUT = PER − ZHANKONBI**（见 user_timer.c）。
 * **实测本板：负载端平均电压与 ZHANKONBI 同向变化**（加压 → 增大 ZN）；电压闭环写 **ZHANKONBI = dt**，其中
 * dt≈(Vcmd_phys/Vbus_phys)×PER，V 由 `MOTOR_ADC_CMD_TO_DISPLAY_VOLT_FLT` / `GET_PHYSICAL_V(valtage_up)` 得到。
 * 自检/压低开关噪声仍可设 ZHANKONBI=PER，使 OUT_PUT=0（与波形边沿一致，勿与负载伏特的直觉混用）。 */
