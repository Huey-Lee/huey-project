/*
 * Function: 电机控制参数、PWM/保护阈值、状态枚举与核心结构体声明。
 * Method:   集中宏配置与 ctr_t/motor_t；被 motor.c、motor_drive、定时器与通信模块包含。
 * Name:     Huey
...
#define SAFE_MIN_VBUS_ADC        (1000u)
#endif
#ifndef MOTOR_VBUS_ADC_FALLBACK_DEN
#define MOTOR_VBUS_ADC_FALLBACK_DEN  (2468u)
#endif

#ifndef BUS_V_220V_THRESHOLD_V
#define BUS_V_220V_THRESHOLD_V   (200u)
#endif
#ifndef MAX_RUN_V_110V
#define MAX_RUN_V_110V           (100u)
#endif
#ifndef MAX_RUN_V_220V
#define MAX_RUN_V_220V           (180u)
#endif
#ifndef E05_ABS_VOLT_110V
#define E05_ABS_VOLT_110V        (220u)
#endif
#ifndef E05_ABS_VOLT_220V
...
/* g_volt_scale：×100 语义（与旧 MOTOR_DISPLAY_VOLT_SCALE_X100 一致），恒为 **PHYS_VOLT_SCALE×100**。
 * 显示/上位「伏」↔ ADC_cmd 经 TF/INV 成对换算；110/220 仅用于限幅与 E05，不再改增益。 */
extern float g_volt_scale;

#define MOTOR_DISPLAY_VOLT_TF_TO_ADC_CMD_FLT(display_volt_tf) \
    ((float)(display_volt_tf) * (g_volt_scale) * 0.01f)

/* ADC_cmd 标尺 → 显示伏特；与 GET_PHYSICAL_V(valtage_up) 配对算 dt=Vcmd/Vbus×PER */
#define MOTOR_ADC_CMD_TO_DISPLAY_VOLT_FLT(adc_cmd_) \
    ((float)(adc_cmd_) * 100.0f / (g_volt_scale))
...
#define MOTOR_MIN_VOLT_DISPLAY_TO_CMD_ADC(DISPLAY_V) \
    ((u16)((float)(DISPLAY_V) * (g_volt_scale) / 100.0f + 0.5f))

extern u16 motor_cmd_min_volt_adc_floor(void);  /* vmin 刻度 × g Volt·scale（四舍五入），与 RUN 路径一致 */

/*──────── 电压标尺拆分（读写维护时请先对号入座） ────────
 * A) 「显示伏特刻度 / 上位机 B」→ 目标电压：**g_volt_scale=PHYS_VOLT_SCALE×100** → **ADC_cmd**（占空 * duty=Vadc×PER/vbus）。
...
/* ZHANKONBI：与 MOTOR_PWM_PERIOD_TICKS 同源；user_timer 写 OUT_PUT = PER − ZHANKONBI。
 * 闭环 dt≈(Vcmd/Vbus)×PER；自检可令 ZHANKONBI=PER 使 OUT_PUT=0。起步 ZN 由参考周期标定 tick 经 MOTOR_PWM_SCALE_REF_TO_PER 换到当前 PER。 */
#ifndef MOTOR_PWM_REF_PERIOD_TICKS
#define MOTOR_PWM_REF_PERIOD_TICKS        (545u)
#endif
#ifndef MT_START_PWM_REF_TICKS
#define MT_START_PWM_REF_TICKS            (25u)
#endif
#ifndef MT_START_ABSMIN_REF_TICKS
#define MT_START_ABSMIN_REF_TICKS         (18u) /* 有输出的下限，躲静摩擦嗒嗒区 */
#endif
#define MOTOR_PWM_SCALE_REF_TO_PER(ref_) \
    ((u16)(((u32)(ref_) * (u32)(MOTOR_PWM_PERIOD_TICKS) + ((MOTOR_PWM_REF_PERIOD_TICKS) / 2u)) \
         / (u32)(MOTOR_PWM_REF_PERIOD_TICKS)))
#define MT_START_PWM           MOTOR_PWM_SCALE_REF_TO_PER(MT_START_PWM_REF_TICKS)
#define MT_START_ABSMIN_DUTY   MOTOR_PWM_SCALE_REF_TO_PER(MT_START_ABSMIN_REF_TICKS)
...
#endif
/* RUN 段 IR：motor_drive_isr 与主环共用以下 Kiv 渐乘/帽、过流 Q 格式宏（统一定义于此，勿在 motor_drive.c 重复）。 */
...
/* 非 0：关闭 E02（不静默、不压差积分、不报 ERROR_02）。默认 0u = 开启 E02；Keil 可 -DMOTOR_E02_DISABLE=1 关掉。 */
#ifndef MOTOR_E02_DISABLE
#define MOTOR_E02_DISABLE  (0u)
#endif

/* 非 0：RUN/STOP 的 ISR 不叠加 Kiv（ΔI→电压）前馈；占空仅 v_exec 经 Vcmd/Vbus 开环换算。START 本无 Kiv。
 * 关闭前馈：Keil 定义 MOTOR_KIV_FEEDFORWARD_DISABLE=1；默认 0u = 启用 Kiv。 */
#ifndef MOTOR_KIV_FEEDFORWARD_DISABLE
#define MOTOR_KIV_FEEDFORWARD_DISABLE  (0u)
#endif

/* E02：STATUS_POWERON 分阶段（仅 MOTOR_E02_DISABLE 为 0 时累计积分判错）：
 * A) MOTOR_E02_POWERON_MUTE_MS：主环每拍 ZHANKONBI=PER、delay1ms，避开上电毛刺与 ADC 脏区；
 * B) 到点：clear_adcbuf，irq_flag/adc_batch_seq 清零；
 * C) 100 批采样：I_offset + motor_e02_bus_diff_integral_add（30% 相对 v_nom）对 MOTOR_E02_INTEGRAL_LIMIT。 */
#ifndef MOTOR_E02_POWERON_MUTE_MS
#define MOTOR_E02_POWERON_MUTE_MS      (1500u)
#endif
#ifndef MOTOR_E02_POWERON_SETTLE_MS
#define MOTOR_E02_POWERON_SETTLE_MS    (300u) /* 保留兼容；POWERON 已用 MUTE_MS */
#endif
#ifndef MOTOR_E02_BUS_VNOM_MIN_V
#define MOTOR_E02_BUS_VNOM_MIN_V       (5u)
#endif
#ifndef MOTOR_E02_BUS_DIFF_PCT_NUM
#define MOTOR_E02_BUS_DIFF_PCT_NUM     (30u)
#endif
#ifndef MOTOR_E02_BUS_DIFF_PCT_DEN
#define MOTOR_E02_BUS_DIFF_PCT_DEN     (100u)
#endif
#ifndef MOTOR_E02_INTEGRAL_LIMIT
#define MOTOR_E02_INTEGRAL_LIMIT       (250u)
#endif

/* 错误码枚举（自检 / 批量 C03 对齐说明见下行内注释）
 * E01 通信中断 → 软停机
 * E02 自检：由 MOTOR_E02_DISABLE 关闭；开启时见 MOTOR_E02_*（MUTE_MS 静默 + 缓冲清洗 + 30% 压差积分）
 * E03 过流/堵转
 * E04 保留
...
typedef enum _error_code_enum
{
    ...
    ERROR_02,      /* 自检：POWERON 静默 + 30% 母线压差积分（见 MOTOR_E02_*） */
    ERROR_03,      /* 过流或堵转 */
    ERROR_04,      /* 保留（老码继电器短等） */
...
{
    STATUS_POWERON = 1,   /* 上电 ADC 自检：E02 由 MOTOR_E02_DISABLE 控制；关时仍采 I_offset */
    STATUS_RUN,           /* 启动门闩：已转 CTR_CLEARED + waitforamoment；E02 仅 POWERON */
...
typedef struct _t_motor
{
    ...
    u8  adjust_min_voltage;     /* 上控下发的最低运行电压（显示刻度）；转 ADC≈ DISPLAY_V×(g_volt_scale/100) */
