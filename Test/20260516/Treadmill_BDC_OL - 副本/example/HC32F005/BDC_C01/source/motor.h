 ...
 * 闭环：dt≈(Vcmd_phys/Vbus_phys)×PER，ZHANKONBI=dt（与实测输出同向）；Vcmd 由本系数换自 ADC_cmd。
...
/* E02 策略（仅 STATUS_POWERON：先 MOTOR_E02_POWERON_SETTLE_MS 静默，再 ADC/TIM6 采集）：
 * 1) 上电后 delay POWERON_SETTLE_MS，待时钟/模拟前端稳定再开 ADC/TIM6 采样；
 * 2) v_nom=(Up+Dn)/2 为动态母线水平，|Up-Dn| 超过 v_nom×(NUM/DEN) 时累计 (|Up-Dn|-门限)；
 * 3) 累计超过 INTEGRAL_LIMIT 判 E02；v_nom < VNOM_MIN 的采样不计入。 */
#ifndef MOTOR_E02_POWERON_SETTLE_MS
#define MOTOR_E02_POWERON_SETTLE_MS    (300u)
#endif
 ...
 * E01 通信中断 → 软停机
 * E02 自检：上电 MOTOR_E02_POWERON_SETTLE_MS 静默后，30% 母线动态压差积分超 MOTOR_E02_INTEGRAL_LIMIT
 * E03 过流/堵转
 * E04 保留
...
typedef enum _error_code_enum
{
    ...
    ERROR_02,      /* 自检：300ms 静默 + 30% 母线压差积分（见 MOTOR_E02_*） */
    ERROR_03,      /* 过流或堵转 */
    ERROR_04,      /* 保留（老码继电器短等） */
...
{
    STATUS_POWERON = 1,   /* 上电 E02：静默 POWERON_SETTLE_MS → ADC 100 点 + 30% 压差积分 */
