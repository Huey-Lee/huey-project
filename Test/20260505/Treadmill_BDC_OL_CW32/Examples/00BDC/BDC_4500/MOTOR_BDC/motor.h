...
/* Tim6(ADTIM6) 三角波：f_pwm ≈ PCLK/(4×PER)。默认 PER=545 @24 MHz≈11 kHz；
 * 需其他载频时在 Keil Define 覆盖 MOTOR_PWM_PERIOD_TICKS。 */
#ifndef MCU_PCLK_HZ_APP
#define MCU_PCLK_HZ_APP (48000000u)
#endif
#ifndef BDC_PWM_PRESCALER
#define BDC_PWM_PRESCALER (7u) /* 分频后 TIM 计数频率 = PCLK/(PSC+1) */
#endif
#ifndef MOTOR_PWM_PERIOD_TICKS
#define MOTOR_PWM_PERIOD_TICKS (545u)
#endif
#ifndef MOTOR_TIM6_ISR_HZ_APPROX
#define MOTOR_TIM6_ISR_HZ_APPROX \
    ((MCU_PCLK_HZ_APP) / ((BDC_PWM_PRESCALER) + 1u) / (MOTOR_PWM_PERIOD_TICKS))
#endif
/* CW32 用边沿对齐 PWM，ISR 频率等于 PWM 频率（与 TIM6 UDF 节拍等效） */
 ...
 * duty = ADC_cmd × PER / motor_vbus_adc；若万用表端压系统性偏低多为该系数偏小（原 813≈×8.13）。
 ...
 * 电压→占空：motor_drive.c 为纯线性——ADC 节拍内 duty_ticks = Vadc × mul >> MOTOR_VBUS_DUTY_LIN_SHIFT，
 * 其中 mul=(PER<<SHIFT)/vbus 仅在母线锁存快照时做一次除；禁止在此再混用旧的 PER÷vbus slow div 心智模型写常数。
...
/* 母线电压 (V) → ADC 计数 */
#define CALC_VOLTAGE_2_ADC(VOTAGE)    (((VOTAGE / MT_ADC_AMP) / 4.95) * 4096)

/* M+ 分压电路：5.1 / (255 + 255 + 5.1) ≈ 1/101；放大倍数 103.2044；参考源 5V */
#define CALC_ADC_2_VOLTAGE(ADC)       (103.2044 * (5 * ADC) / 4096)

/* 采样电阻（MOS + 周围电路）信号有效增益约 1.1 倍 */
#define CALC_ADC_2_CURRENT(ADC)       ((((ADC / 4096.0) * 4.95) / MT_CURRENT_ADC_AMP) / (SAMPLE_RES))
