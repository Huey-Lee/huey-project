 ...
 */
static uint16_t uart_kmh_payload_to_speed_scale(uint8_t kbuf)
{
    if (kbuf < 10u)
        kbuf = 10u;
    if (kbuf > 100u)
        kbuf = 100u;
    return (uint16_t)kbuf * 10u;
}

T_QUEUE rx_queue;                  /* 串口接收环形队列 */
uint8_t rx_buf[RX_BUFF_SIZE];      /* 接收缓冲区（135 字节） */
...
{
    (void)UARTPort;
    BDC_Uart_SendBytes(ptr, len);
}

/* 发送单字节数据帧：[SOF][CMD][LEN=1][DAT][XOR][0x00][EOF]
 * XOR = CMD ^ LEN ^ DAT，用于接收端校验 */
void uart_frame_tx(uint8_t cmd, uint8_t dat)
{
    uint8_t xorval = 0;
    ...
    UART_Send_Buf(UART0, (uint8_t *)&uart_tx_frame, uart_tx_frame.len + 6);
}

/* 发送双字节数据帧：[SOF][CMD][LEN=2][DAT0][DAT1][XOR][0x00][EOF] */
void uart_frame_tx_2(uint8_t cmd, uint8_t dat0, uint8_t dat1)
{
    uint8_t xorval = 0;
    uart_tx_frame.cmd    = cmd;    xorval ^= cmd;
...
void communication_checkout(void)
{
    if (ctr.ack == 1) {
        if (ctr.error_code) {
            ...
            uart_frame_tx(CMD_ACK, 2);                 /* 正常应答 */
        }
        ctr.ack = 0;
    }
}

extern uint8_t uart_time_error;
...
static uint8_t ClampKiv(uint8_t raw)
{
...
void cmd_proc(void)
{
        ...
        if (motor.en != uart_frame.buf[0]) {
            if (uart_frame.buf[0] == 0) {
                ...
                if (uart_frame.len >= 2u) {
                    uint16_t ss = uart_kmh_payload_to_speed_scale(uart_frame.buf[1]);
                    ...
                    if (motor.set_speed_scale < (uint16_t)(motor.sta_speed_scale))
    ...
    case CMD_VOLTAGE_MAX:
        if (uart_frame.len < 1u)
            break;
        if (uart_frame.buf[0] > rx_setparam.voltage_min) {  /* 必须高于最低电压 */
    ...
    case CMD_OVER_CURRENT_MAX: {
        if (uart_frame.len < 1u)
            break;
        uint32_t raw_val = (uint32_t)uart_frame.buf[0] * 10u;

        if (raw_val < (uint32_t)MOTOR_OVER_CURRENT_ADC_FLOOR)
            raw_val = (uint32_t)MOTOR_OVER_CURRENT_ADC_FLOOR;
        if (raw_val > (uint32_t)MOTOR_OVER_CURRENT_ADC_CEIL)
            raw_val = (uint32_t)MOTOR_OVER_CURRENT_ADC_CEIL;
        motor.adjust_ove_curren = (uint16_t)raw_val;
