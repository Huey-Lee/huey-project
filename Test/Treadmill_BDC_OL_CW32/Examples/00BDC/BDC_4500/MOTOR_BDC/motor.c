...
extern uint16_t ZHANKONBI;  /* 当前 PWM 占空比（写入 Tim6 比较寄存器） */
...
{
    static u8 interrupt_flag = 0, er02_code = 0, gate_dual_low_cnt = 0;
    u16       val_up, val_down;
    ...
    {
        u32 cur_sum = 0u;
        /* 每轮门控独立计数，避免 static 状态从上次自检/故障路径泄漏 */
        interrupt_flag = 0u;
        er02_code          = 0u;
        gate_dual_low_cnt  = 0u;
        user_adc_init();
        for (u8 i = 0u; i < 100u; i++) {
            if (adc_handle.irq_flag == 0u && interrupt_flag == 0u)
                Adc_Start();
            if (interrupt_flag != adc_handle.irq_flag) {
                if (adc_handle.irq_flag > 0u) {
    ...
    case STATUS_TO_RUN:
        ...
        if (!bdc_pwm_running() || !bdc_relay_on_readback()) {
            motor.status             = STATUS_MT_START;
