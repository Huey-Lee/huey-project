...
//				printf("ctr:%d\r\n",ctr.status);
	...
	case STATUS_ERROR:
        /* 统一分级：
         * Class-1: E02/E03/E05/E06/E08 -> 立即切断PWM
         * Class-2: E01 -> 受控平滑减速
         */
        switch (ctr.error_code)
        {
            case ERROR_02:
            case ERROR_03:  /* over-current / stall: immediate cut */
            case ERROR_05:
            case ERROR_06:  /* over-speed / voltage overshoot: immediate cut */
            case ERROR_08:  /* MOS short-circuit: immediate cut */
                motor_immediate_pwm_cutoff();
                break;
            case ERROR_01:
...
/********************************************************************************************************/
		...
		if(cut_adc<0)
			cut_adc=0;
		now_valtage = (u16)(0.126f * cut_adc);

		motor.adjust_sta_voltage += motor.start_add_val;
		if (motor.adjust_sta_voltage > START_VOLTAGE_MAX_ADC) {
			motor.adjust_sta_voltage = START_VOLTAGE_MAX_ADC;
		}
		/* Hard cap: open-loop target must never exceed the closed-loop cruise target.
		 * This guarantees cut_adc ≤ adjust_now_voltage at the START→PID handoff,
		 * so the PID setpoint never needs to DROP, eliminating the inductive flyback
		 * spike (50-60 V flash) seen at the motor terminal. The upper controller's
		 * start level still controls how quickly and steeply the voltage climbs to
		 * the cap (higher level = faster, stronger ramp). */
