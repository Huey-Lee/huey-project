...
/* 下控 FC01/00 中 Data1 大端档位（0.1 km/h，下发表 TM_DOWNLINK_*）；data[2]=运行状态 */
static uint16_t s_lower_spd_fbk;
static uint8_t  s_lower_status_fbk;
/* 启动：待「已跑」确认（状态9 或 反馈速≥下发表最低档），超时补发一轮 RUN+档位 仅一次 */
...
/* 显示目标速度 → 下控下发速度（线性、四舍五入到 0.1 km/h） */
static uint16_t Speed_DisplayToDownlink(uint16_t disp)
{
    if (disp <= TM_SPEED_MIN) {
        return TM_DOWNLINK_SPEED_MIN;
    }
    if (disp >= TM_SPEED_MAX) {
        return TM_DOWNLINK_SPEED_MAX;
    }
    {
        uint32_t num = (uint32_t)(disp - TM_SPEED_MIN) *
                       (uint32_t)(TM_DOWNLINK_SPEED_MAX - TM_DOWNLINK_SPEED_MIN);
        uint32_t den = (uint32_t)(TM_SPEED_MAX - TM_SPEED_MIN);
        return (uint16_t)(TM_DOWNLINK_SPEED_MIN + (num + den / 2u) / den);
    }
}

/* 下控反馈速度（下发表）→ 显示速度，与 Speed_DisplayToDownlink 互逆 */
static uint16_t Speed_DownlinkToDisplay(uint16_t low)
{
    if (low == 0u) {
        return 0u;
    }
    if (low <= TM_DOWNLINK_SPEED_MIN) {
        return TM_SPEED_MIN;
    }
    if (low >= TM_DOWNLINK_SPEED_MAX) {
        return TM_SPEED_MAX;
    }
    {
        uint32_t num = (uint32_t)(low - TM_DOWNLINK_SPEED_MIN) *
                       (uint32_t)(TM_SPEED_MAX - TM_SPEED_MIN);
        uint32_t den = (uint32_t)(TM_DOWNLINK_SPEED_MAX - TM_DOWNLINK_SPEED_MIN);
        return (uint16_t)(TM_SPEED_MIN + (num + den / 2u) / den);
    }
}
...
/* 档位：Data1 = 目标档位 16 位大端，单位为下控下发表（TM_DOWNLINK_*，0.1 km/h） */
...
/* spd 为显示目标（TM_SPEED_MIN..MAX），内部转换为下发表再发 */
static void Send_Speed(uint16_t spd_disp)
{
    Send_GearWord(Speed_DisplayToDownlink(spd_disp));
}
...
static void Send_Run(uint16_t speed_disp)
{
    uint8_t i;
    uint16_t low = Speed_DisplayToDownlink(speed_disp);
...
void AeroLink_OnFrameReceived(AeroFrame_t *f)
{
    ...
    /* 故障信息：FC 0x01 / SFC 0x01，Data1 一级、Data2 二级 */
    ...
    /* 运行信息：FC 0x01 / SFC 0x00，Data1=档位反馈（大端，下发表），Data2=运行状态 */
...
static void Adjust_Target(int8_t dir, bool is_hold)
{
    ...
    switch (g_TM.disp_index) {
        ...
        case DISP_SPEED: {
            ...
            if (dir > 0) {
                if (g_TM.speed >= TM_SPEED_MAX) break; /* 已在上限，Keypad 侧可再响一声 */
                ...
                if (!is_hold || g_TM.speed == TM_SPEED_MAX) {
                    Beep_Play(BEEP_CLICK);
                ...
                if (!is_hold || g_TM.speed == TM_SPEED_MIN) {
                    Beep_Play(BEEP_CLICK);
                    s_suppress_keypad_beep = 1;
                }
            }
...
static void LowerStartAck100ms(void)
{
    ...
    if (s_lower_status_fbk == TM_LOWER_STATUS_RUNNING ||
        s_lower_spd_fbk >= TM_DOWNLINK_SPEED_MIN) {
        s_start_ack_pending    = 0u;
        s_start_retried        = 0u;
        s_start_ack_wait_100ms = 0u;
        return;
...
static void Stop_Tick(void)
{
    if (s_lower_spd_fbk > 0u) {
        ...
        uint16_t fbk_disp = Speed_DownlinkToDisplay(s_lower_spd_fbk);
        uint16_t fbk_c    = (fbk_disp < s_stop_start_spd) ? fbk_disp : s_stop_start_spd;
